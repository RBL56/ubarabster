...
const AppContent = observer(() => {
    ...
    const global_timeout_ref = React.useRef(null);
    ...
    // Unified Global Safety Timeout
    useEffect(() => {
        if (is_loading) {
            console.log('[AppContent] Starting global safety timeout (15s)');
            global_timeout_ref.current = setTimeout(() => {
                if (is_loading) {
                    console.warn('[AppContent] Global timeout reached, forcing dashboard visibility');
                    setIsLoading(false);
                    setIsApiInitialized(true);
                    if (!app.dbot_store) {
                        init();
        ...
        return () => {
            if (global_timeout_ref.current) {
                clearTimeout(global_timeout_ref.current);
            }
        };
    }, [is_loading, app.dbot_store]);
